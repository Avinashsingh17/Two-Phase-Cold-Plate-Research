---
title: "{{Author Year}} — {{Short Title}}"
type: paper
source: "{{Full citation}}"
date_ingested: YYYY-MM-DD
tags: []
---

## Summary

{{2–4 sentence overview of the paper's contribution.}}

## Key Findings

- {{Finding 1}} [Author YYYY]
- {{Finding 2}} [Author YYYY]

## Experimental / Computational Setup

| Parameter | Value |
|-----------|-------|
| Geometry  | {{e.g., rectangular channel, 10 mm × 2 mm}} |
| Fluid     | {{e.g., DI water}} |
| Heat flux range | {{e.g., 0–200 W/cm²}} |
| Mass flux range | {{e.g., 200–1000 kg/m²s}} |
| Pressure  | {{e.g., 1 atm}} |

## Models / Correlations Used

- {{List key correlations or modeling approaches with citations}}

## Data Available for Validation

- {{Describe figures, tables, or datasets that can serve as benchmarks}}

## Applicability to This Project

{{How does this paper connect to the cold plate optimization goals? Then:
explicit assessment of how directly this paper's data, correlations, or models
can be applied to the cold plate geometry, fluid, and operating conditions.
Flag geometry mismatches (e.g., round tube vs. rectangular channel), fluid
mismatches, or parametric range gaps. Note standard workarounds (e.g.,
hydraulic diameter substitution) and the extra uncertainty they introduce.
Cross-reference papers that fill the gaps.}}

## Cross-References

- [[RelatedConcept]]
- [[RelatedEntity]]

## Notes

- <!-- TODO: verify --> {{Any uncertain claims}}
